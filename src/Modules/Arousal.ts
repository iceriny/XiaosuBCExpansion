...
export default class ArousalModule extends BaseModule {
    ...
    get getEndureDesc(): string {
        return this.descriptionOfEnduranceActivities[
            Math.floor(Math.random() * this.descriptionOfEnduranceActivities.length)
        ];
    }
    public Run(): void {
        // 处理本模块的Hook函数
    ...
    private HookList(): void {
        ...
        HookManager.setHook("ChatRoomSync", "SyncArousalData", -10, () => {
        ...
        HookManager.setHook("Player.GetSlowLevel", "aftertasteWeaknessEffect", -9, (args, lastResult) => {
            if (Player.RestrictionSettings?.SlowImmunity) return { args: args, result: 0 };
        ...
        HookManager.setHook("CommandParse", "aftertasteEffectAboutChat", 10, (args) => {
            if (this.afterEffectSwitch.relax) {
                ...
                if (
                    ...
                    if (segmentList === null) {
                        conDebug(
                            "程序正在处理 消息分词，但您的浏览器不支持该功能!!! 无法显示余韵的特殊字符串加工效果。"
                        );
                    } else {
                        let cacheStr = "";
    ...
    private msgSubStringHandle(subStr: string): string {
        // 获取三个概率判断结果
        const probability_bold =
            Math.random() < getDynamicProbability(this._aftertaste, 0, ArousalModule.MAX_AFTERTASTE, 0.2, 0.8);
        const probability_moan =
            Math.random() < getDynamicProbability(this._aftertaste, 0, ArousalModule.MAX_AFTERTASTE, 0.1, 0, true);
        const probability_negative =
            Math.random() < getDynamicProbability(this._aftertaste, 0, ArousalModule.MAX_AFTERTASTE, 0.05, -0.02, true);
    ...
    private TimerProcess(): void {
        ...
        TimerProcessInjector.add(
            ...
            45000,
            () => {
                return !!Player.ArousalSettings && Player.ArousalSettings!.Progress >= 93;
            },
            {
                code: () => {
        ...
        TimerProcessInjector.add(
            ...
            () => {
                return (
                    ...
                    Player.ArousalSettings!.Progress < 60 &&
    ...
    private patchListHandler(): void {
        ...
        HookManager.patchAdd("ActivityOrgasmStart", {
            ...
            "C.ArousalSettings.OrgasmTimer = CurrentTime + (Math.random() * 10000) + 5000;": `if (Player.XSBE && C.IsPlayer()) {
    ...
    private afterEffectDescribe: { [name in AftertasteEffect]: string[] } = {
        relax: [
            `${PH.s}的身体在快感冲击下软了下来。浑身软绵绵的使不上力气。`,
            `${PH.s}的身体渐渐恢复，重新恢复了正常的体力。`,
        ],
        weakness: [
            `${PH.s}的身体在连续的快感冲击中变得越来越软，越来越无力，已经难以控制自己的身体。`,
            `${PH.s}的身体虽然还是软绵绵的，但大概可以正常走路了。`,
        ],
        twitch: [
            `连续深度的高潮冲击下，${PH.s}的身体不自觉的开始抽搐，再这么下去...`,
            `${PH.s}停止了抽搐，但还是很难支撑起自己的身体。`,
        ],
        trance: [
            `高潮过于猛烈，${PH.s}的脑袋已经不清楚了。恍恍惚惚，意识断断续续。想要移动的话..应该需要很大的意志力才能挪动吧。`,
            `${PH.s}的意识恢复了，但身体还是时不时的抽动一下。`,
        ],
        absentminded: [
            `强大的、连续的、不可抵挡的高潮冲击下，${PH.s}已经完全无法控制自己的身体。只能在无意识中抽搐着身体，发出细软的声音。这样的话...`,
