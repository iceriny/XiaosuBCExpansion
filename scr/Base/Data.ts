...
export class Data<T extends IData> {
    ...
    constructor(private data: T, onlineKeys: (keyof T)[], settingKeys: (keyof T)[], localKeys: (keyof T)[]) {
        ...
        // 比对服务器数据和本地数据谁更新
        // 获取服务器数据
        const serverExtensionSettingObject = this.getExtensionSettings() as Record<keyof T, T[keyof T]>;
        // 获取服务器数据的时间戳
     ...
     */
    set<K extends keyof T>(key: K, value: T[K], upload: boolean = false) {
        this.data[key] = value;
    ...
    getLocalStorage<K extends keyof T>(): object {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            const value = JSON.parse(localStorage.getItem(key) as string) as T[K];
            (localStorageObject as Record<K, T[K]>)[key as K] = value;
        }
